...
NodePtr Norm(const Value& input, const c10::optional<at::Scalar>& p,
  ...
  auto dimensions = torch::lazy::ToVector<xla::int64_t>(dims);
  if (dimensions.empty()) {
    dimensions = torch::lazy::Iota<xla::int64_t>(input.shape().rank());
  }
...
NodePtr MaxUnary(const Value& input) {
  auto lower_fn = [](const Node& node, LoweringContext* loctx) -> XlaOpVector {
    ...
    xla::XlaOp result = xla::Reduce(
        ...
        torch::lazy::Iota<xla::int64_t>(input_shape.rank()));
